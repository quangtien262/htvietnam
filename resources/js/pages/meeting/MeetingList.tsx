...
import { DATE_FORMAT, DATE_TIME_SHOW } from '../../function/constant'

import { taskInfo } from "../task/task_config";
import { projectInfo } from "../projects/project_config";
...
const MeetingList: React.FC = () => {
    ...
    const [meetingStatus_select, setMeetingStatus_select] = useState([]);
    ...
    function fetchData(request = {}) {
        axios.post(API.meetingIndexApi, request).then((res) => {
            console.log('Fetch', res.data.data);
            const propsTmp = res.data.data;
            setProps(propsTmp);
            setDataSource(propsTmp.datas);
            setMeetingStatus(propsTmp.meetingStatus);
            setMeetingStatus_select(propsTmp.meetingStatus_select);
            setUsers(propsTmp.users);
            setSearchData(propsTmp.searchData);
            setTableParams({
                pagination: {
                    ...tableParams.pagination,
                    total: propsTmp.pageConfig.total,
                    pageSize: propsTmp.pageConfig.per_page,
                    current: propsTmp.pageConfig.current_page,
                },
            });
            formSearch.setFieldsValue(propsTmp.searchData);
        }).catch((err) => {
            console.error(err);
        });
    }
    ...
    function search(values: any) {
        ...
        axios.post(API.meetingSearch, values).then((res) => {
            const result = res.data.data;
            setDataSource(result.datas);
            setTableParams({
                pagination: {
                    ...tableParams.pagination,
                    total: result.pageConfig.total,
                    pageSize: result.pageConfig.per_page,
                    current: result.pageConfig.current_page,
                },
            });
            message.success("Đã cập nhật lại dữ liệu");
        }).finally(() => {
                ...
                <Row gutter={24} className="main-search-left">
                    ...
                    <Col span={24}>
                        <Form.Item name='is_recycle_bin' label=''>
                            <Checkbox onChange={(e) => {
                                formSearch.setFieldValue('is_recycle_bin', e.target.checked ? 1 : 0);
                                formSearch.submit();
                            }}><b>Tìm trong thùng rác</b></Checkbox>
                        </Form.Item>
                    </Col>
                    <Col span={24}>
                        <br />
    ...
    function formAddExpress() {
        ...
        function addExpress() {
            ...
            axios.post(API.meetingAddExpress, {
    ...
    function getTaskInfo(taskId: number) {
        ...
        axios.post(API.taskInfo, { id: taskId }).then((response) => {
            const res = response.data.data;
            setTaskChecklist(res.checklist);
            setTaskChecklistPercent(res.percent);
            setTaskComments(res.comments);
            setTaskAction(res.task);
            setPriority(res.priority);
            setTaskLog(res.taskLog);
            setTaskStatus(res.status);
            console.log('Task Info:', res.status);
        });
    }
    ...
    function getProjectInfo(projectId: number) {
        ...
        axios.post(API.projectGetInfo, { project_id: projectId }).then((response) => {
            const res = response.data.data;
            console.log('res', res);
            setProjectChecklist(res.checklist);
            setProjectChecklistPercent(res.percent);
            setProjectComments(res.comments);
            setProjectDataAction(res.project);
            setProjectStatus(res.status);
            setProjectSearchData({ display: 'list', parentName: res.project.parent_name });
        });
    }
    ...
    function onClickItem(record: any) {
        if (record.data_type === 'projects') {
            ...
            getProjectInfo(record.project_id);
        }
        if (record.data_type === 'tasks') {
            getTaskInfo(record.task_id);
        }
    }
            ...
            <Modal title="Cập nhật meeting"
                ...
                <Form form={formMeetingEdit}
                    ...
                    onFinish={(values) => {
                        ...
                        // call api
                        axios.post(API.meetingUpdate, values).then((res) => {
            ...
            >
                ...
                {projectInfo(props,
                    ...
                    (result: any) => {
                        ...
                        getProjectInfo(projectAction.id);
