...
const DienNuoc: React.FC = () => {
    ...
    // Lấy tất cả query parameters từ URL và gán vào searchData
    const getAllQueryParams = () => {
        const allParams: { [key: string]: any } = {};
        params.forEach((value, key) => {
            // Xử lý array parameters (những tham số có [] ở cuối)
            if (key.endsWith('[]')) {
                const cleanKey = key.slice(0, -2);
                if (!allParams[cleanKey]) {
                    allParams[cleanKey] = [];
                }
                allParams[cleanKey].push(value);
            } else {
    ...
    useEffect(() => {
        ...
        setSearchData(allParams);

        // Fetch data với các tham số từ URL
        fetchData(allParams);
    }, []);

    // Thêm useEffect để theo dõi sự thay đổi của searchData
    useEffect(() => {
    ...
    function search(request = {}) {
        // Tạo query string từ request object
        const queryParams = new URLSearchParams();

        Object.entries(request).forEach(([key, value]) => {
            if (value !== null && value !== undefined && value !== '') {
                if (Array.isArray(value)) {
                    value.forEach((item: any) => queryParams.append(`${key}[]`, item));
                } else {
        ...
        // Navigate đến URL mới với query parameters
        const currentPath = window.location.pathname;
        const newUrl = `${currentPath}?${queryParams.toString()}`;

        // Sử dụng window.location để reload trang với query mới
    ...
    const inFinishSearch = (values: any) => {
        ...
        if (!values.month) {
            values.month = searchData.month;
        }
        if (!values.year) {
            values.year = searchData.year;
        }
        if (!values.room && searchData.room) {
            values.room = searchData.room;
        }
        if (isRecycleBin) {
    ...
    function initialsFormSearch() {
        ...
        if (searchData && searchData.month && searchData.year) {
            result.date = dayjs(searchData.year + '-' + String(searchData.month).padStart(2, '0'));
        }
        return result;
    }
    ...
    function openQuickEditFromSelection() {
        ...
        const monthYearSet = new Set(
            selectedRecords.map((r: any) => {
                const m = Number(r?.month ?? searchData?.month);
                const y = Number(r?.year ?? searchData?.year);
                return `${y}-${m}`;
            })
        );
    ...
    return (
        <div>
            {/* Debug: Hiển thị thông tin searchData */}
            {Object.keys(searchData).length > 0 && (
                <div style={{ padding: '10px', backgroundColor: '#f5f5f5', margin: '10px 0', fontSize: '12px' }}>
                    <strong>Current Search Parameters:</strong>
                    <pre>{JSON.stringify(searchData, null, 2)}</pre>
                </div>
            )}

            <Divider orientation="left">
                Danh sách số điện/nước {searchData && searchData.month && searchData.year ? `tháng ${searchData.month}/${searchData.year}` : ''}
            </Divider>
