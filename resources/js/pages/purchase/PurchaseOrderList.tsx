...
const PurchaseOrderList: React.FC = () => {
    ...
    const loadHangHoa = async () => {
        try {
            const res = await axios.post(API.hangHoaActive, {});
            if (res.data.status_code === 200) {
                setHangHoaList(res.data.data);
            }
        } catch (error) {
                            ...
                            <>
                                <div style={{ marginBottom: 16 }}>
                                    <Table
                                        ...
                                        columns={[
                                            {
                                                title: <span>Hàng hóa <span style={{ color: '#ff4d4f' }}>*</span></span>,
                                                key: 'hang_hoa_id',
                                                width: '30%',
                                                render: (_, field) => (
                                                    <Form.Item
                                                        name={[field.name, 'hang_hoa_id']}
                                                    ...
                                                    >
                                                        <Select
                                                            ...
                                                            onChange={(hangHoaId) => {
                                                                const hangHoa = hangHoaList.find(h => h.id === hangHoaId);
                                                                if (hangHoa) {
                                                                    ...
                                                                    items[field.name] = {
                                                                        ...
                                                                        unit: hangHoa.unit || 'Cái',
                                                                        quantity: hangHoa.so_luong_default || 1,
                                                                        unit_price: hangHoa.price_default || 0,
                                                                        tax_rate: hangHoa.vat || 0
                                                                    };
                                                        ...
                                                        >
                                                            {hangHoaList.map(h => (
                                                                <Select.Option key={h.id} value={h.id} label={`${h.name} (${h.code})`}>
                                                                    {h.name} ({h.code})
                                                                </Select.Option>
                                                            ))}
                                                        </Select>
