...
export default function Dashboard(props) {
    ...
    const [tongSoLuongHuy, setTongSoLuongHuy] = useState(0);
    const [tongGiaTriHuy, setTongGiaTriHuy] = useState(0);
    ...
    const dataDetail_item_default = {
        ...
        ton_kho_truoc_khi_huy: 0,
        ton_kho_sau_khi_huy: 0
    };
    ...
    const onFinishFormEdit = (values) => {
        ...
        values.so_luong_huy = tongSoLuongHuy;
        values.gia_tri_huy = tongGiaTriHuy;
        ...
        // save
        axios.post(route("saveXuatHuy"), values).then((response) => {
    ...
    const expandedRowRender = (record, index) => {
        ...
        const detail = info.map((item, idx) => {
            return <tr key={idx}>
                ...
                <td>{item.ton_kho_truoc_khi_huy}</td>
                <td>{numberFormat(item.so_luong_huy)}</td>
                <td>{numberFormat(item.ton_kho_sau_khi_huy)}</td>
                <td>{numberFormat(item.gia_von)}</td>
                <td>{numberFormat(item.gia_tri_huy)}</td>
            </tr>
        });
            ...
            <table className="table-sub">
                ...
                <tbody>
                    <tr className="border-none">
                        <td>
                            <a target="new" href={route('print.xuatHuy', [record.id])}>
                                <button className="btn-print"><PrinterOutlined /> In</button>
                            </a>
                        </td>
                        <td colSpan={4} className="text-right01">Tổng SL hủy:</td>
                        <td>{numberFormat(record.so_luong_huy)}</td>
                    </tr>
                    <tr className="border-none">
                        <td>
            ...
            return <tr key={idx}>
                ...
                <td className="td-input">
                    <InputNumber
                        ...
                        value={data.so_luong}
                        min={1}
                        max={data.ton_kho_truoc_khi_huy}
                        onChange={(value) => {
                            let data_tmp = cloneDeep(dataDetail);
    ...
    function tinhTongTien(dataDetail_tmp) {
        let tongSoLuongHuy_tmp = 0;
        let tongGiaTriHuy_tmp = 0;
        dataDetail_tmp.forEach((val) => {
            tongSoLuongHuy_tmp += val.so_luong;
            tongGiaTriHuy_tmp += val.thanh_tien;
        });
        setTongSoLuongHuy(tongSoLuongHuy_tmp);
        setTongGiaTriHuy(tongGiaTriHuy_tmp);
    }
