...
export default function Dashboard(props) {
    ...
    const [DVTrongGoi, setDVTrongGoi] = useState([]);
            ...
            return <tr key={i}>
                ...
                <td>
                    <Form.Item className="item-form so-lan-su-dung" name={d.name}>
                        <InputNumber onChange={(sl)=>updateSLSanPhamTrongGoi(sl, i)} placeholder="Số lần sử dụng" formatter={(value) => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')} parser={(value) => value.replace(/\$\s?|(,*)/g, '')} />
                    </Form.Item>
                </td>
                                ...
                                <Col sm={{ span: 8 }} className="main-item-form">
                                    <div className="sub-main-item-form">
                                        ...
                                        <Form.Item className={'item-form ' + classItem.gia_von } name='gia_von' label='Giá vốn'  rules={[{ required: false,message: 'Giá vốn không dc bỏ trống',}]}>
                                            <InputNumber min={0} onBlur={() => { console.log('blue'); updateSubProduct(); }} />
                                        </Form.Item>
