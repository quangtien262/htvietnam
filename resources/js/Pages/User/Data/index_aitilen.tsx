...
import { Button, Table, message, Modal, Form, Input, InputNumber, Popconfirm,  Select, Row,  Tag, Card, notification } from 'antd';
import { Link } from '@inertiajs/react';
import axios from 'axios';
import { router } from '@inertiajs/react';
import { ArrowRightOutlined, FormOutlined, SearchOutlined, DeleteOutlined, EditOutlined,} from '@ant-design/icons';
import '../../../../css/list.css';
...
const openNotification = (type, msg) => {
    ...
    const onFinishInputFastEdit = (value) => {
        ...
        axios.post(route('data.fastEdit',[props.tableId]),value).then(response => {
            if (response.data.status_code == 200) {
                ...
                openNotification('success', 'Đã lưu thành công!');
    ...
    const pageContent = <div>
        <Card type='inner' title='Tìm kiếm' extra={''}>
            <Form
                name='basic'
                layout='vertical'
            ...
            >
                <Row gutter={24}>{listItemsSearch}</Row>
                <Button
                    type='primary'
                    htmlType='submit'
        ...
        <Form form={form} component={false}>
            <div style={{ marginBottom: 16 }}>
                ...
                >
                    <p>
                        Dữ liệu đã xóa sẽ{' '}
                        <b>không thể khôi phục</b> lại được{' '}
                        <br />{' '}
                        <b>
                            (Số lượng {selectedRowKeys.length})
                        </b>
                    </p>
                </Modal>
                <Button
                ...
                <em>
                    {' '}
                    Trang {props.pageConfig.currentPage}, hiển
                    thị {props.pageConfig.perPage}/
