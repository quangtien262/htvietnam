@extends('layouts.layout01.lay01')

@section('content')

        {!! app('Helper')->btnLandingpageSetting($menuId) !!}

        <div id="content" role="main" class="content-area">
       <section class="section dau-trang-section dark has-parallax" id="section_1870807667">
          <div class="bg section-bg fill bg-fill parallax-active bg-loaded" data-parallax-container=".section" data-parallax-background="" data-parallax="-7" style="height: 760.462px; transform: translate3d(0px, -131.69px, 0px); backface-visibility: hidden;">
             <div class="section-bg-overlay absolute fill"></div>
          </div>
          <div class="section-content relative">
             <div class="row align-middle" id="row-516515262">
                <div id="col-1043523965" class="col medium-6 small-12 large-6">
                   <div class="col-inner">
                      <div class="tieu-de">
                         <h2>ABOUT US</h2>
                      </div>
                   </div>
                </div>
                <div id="col-920517131" class="col medium-6 small-12 large-6">
                   <div class="col-inner text-right">
                      <nav aria-label="breadcrumbs" class="rank-math-breadcrumb">
                         <p><a href="https://gioithieucongty2.ninhbinhweb.site" target="_blank" rel="noopener">Trang chủ</a><span class="separator"> - </span><span class="last">Về chúng tôi</span></p>
                      </nav>
                   </div>
                </div>
             </div>
          </div>
          <style>
             #section_1870807667 {
             padding-top: 60px;
             padding-bottom: 60px;
             }
             #section_1870807667 .section-bg-overlay {
             background-color: rgba(0, 0, 0, 0.924);
             }
             #section_1870807667 .section-bg.bg-loaded {
             background-image: url(https://solar3.maugiaodien.com/wp-content/uploads/2023/01/img3.jpg);
             }
             #section_1870807667 .ux-shape-divider--top svg {
             height: 150px;
             --divider-top-width: 100%;
             }
             #section_1870807667 .ux-shape-divider--bottom svg {
             height: 150px;
             --divider-width: 100%;
             }
          </style>
       </section>
       <section class="section" id="section_126874805">
          <div class="bg section-bg fill bg-fill bg-loaded">
          </div>
          <div class="section-content relative">
             <div class="row" id="row-246282350">
                <div id="col-596972795" class="col medium-7 small-12 large-7">
                   <div class="col-inner">
                      <p>WE OFFER GREEN TECHNOLOGIES</p>
                      <h2><span style="font-size: 130%;">ABOUT OUR FACTORY</span></h2>
                      <p><em><strong>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce id nibh non lectus finibus tempor. Aliquam erat volutpat. Morbi egestas mi vitae dui pretium placerat. Aenean tristique sapien.</strong></em></p>
                      <p>Quisque maximus dolor non accumsan accumsan. Praesent mollis velit sed libero aliquam, sed aliquam sapien lacinia. Sed suscipit eros nec ultrices molestie. Vestibulum mollis purus in metus faucibus, in dapibus ante dignissim. Duis convallis nulla id nisi imperdiet, nec vehicula augue semper. Donec sed pharetra est, vel congue magna. Curabitur ligula mauris, semper at eros id</p>
                      <a href="#" target="_self" class="button primary lowercase nut-xem-them">
                      <span>Read more</span>
                      </a>
                   </div>
                </div>
                <div id="col-858575644" class="col cot2 medium-5 small-12 large-5">
                   <div class="col-inner">
                      <div class="img has-hover x md-x lg-x y md-y lg-y" id="image_68445674">
                         <div class="img-inner dark">
                            <img decoding="async" width="800" height="562" src="https://solar3.maugiaodien.com/wp-content/uploads/2023/01/21.jpg" class="attachment-original size-original" alt="" srcset="https://solar3.maugiaodien.com/wp-content/uploads/2023/01/21.jpg 800w, https://solar3.maugiaodien.com/wp-content/uploads/2023/01/21-300x211.jpg 300w, https://solar3.maugiaodien.com/wp-content/uploads/2023/01/21-768x540.jpg 768w, https://solar3.maugiaodien.com/wp-content/uploads/2023/01/21-600x422.jpg 600w" sizes="(max-width: 800px) 100vw, 800px">						
                         </div>
                         <style>
                            #image_68445674 {
                            width: 100%;
                            }
                         </style>
                      </div>
                   </div>
                </div>
             </div>
          </div>
          <style>
             #section_126874805 {
             padding-top: 80px;
             padding-bottom: 80px;
             }
             #section_126874805 .ux-shape-divider--top svg {
             height: 150px;
             --divider-top-width: 100%;
             }
             #section_126874805 .ux-shape-divider--bottom svg {
             height: 150px;
             --divider-width: 100%;
             }
          </style>
       </section>
       <section class="section section4 section41 dark" id="section_225374476">
          <div class="bg section-bg fill bg-fill bg-loaded">
          </div>
          <div class="section-content relative">
             <div class="row" id="row-1967935179">
                <div id="col-1674328134" class="col cot1 medium-3 small-12 large-3">
                   <div class="col-inner text-center">
                      <p><span id="count1" class="display-4">300</span></p>
                      <div class="item-text">
                         <p>FINISHED PROJECTS</p>
                      </div>
                   </div>
                </div>
                <div id="col-165675885" class="col cot1 medium-3 small-12 large-3">
                   <div class="col-inner text-center">
                      <p><span id="count2" class="display-4">250</span></p>
                      <div class="item-text">
                         <p>HAPPY CLIENTS</p>
                      </div>
                   </div>
                </div>
                <div id="col-102733404" class="col cot1 show-for-medium medium-3 small-12 large-3">
                   <div class="col-inner text-center">
                      <p><span id="count2" class="display-4">250</span></p>
                      <div class="item-text">
                         <p>HAPPY CLIENTS</p>
                      </div>
                   </div>
                </div>
                <div id="col-645152751" class="col cot2 medium-3 small-12 large-3">
                   <div class="col-inner text-center">
                      <p><span id="count3" class="display-4">200</span></p>
                      <div class="item-text">
                         <p>SATISFIED EMPLOYEES</p>
                      </div>
                   </div>
                </div>
                <div id="col-1278011945" class="col cot2 medium-3 small-12 large-3">
                   <div class="col-inner text-center">
                      <p><span id="count4" class="display-4">1000</span></p>
                      <div class="item-text">
                         <p>WORKING HOURS</p>
                      </div>
                   </div>
                </div>
                <div id="col-809006233" class="col cot2 medium-3 small-12 large-3">
                   <div class="col-inner text-center">
                      <p><span id="count5" class="display-4">50</span></p>
                      <div class="item-text">
                         <p>WORKING HOURS</p>
                      </div>
                   </div>
                </div>
             </div>
          </div>
          <style>
             #section_225374476 {
             padding-top: 60px;
             padding-bottom: 60px;
             background-color: rgb(51, 51, 51);
             }
             #section_225374476 .ux-shape-divider--top svg {
             height: 150px;
             --divider-top-width: 100%;
             }
             #section_225374476 .ux-shape-divider--bottom svg {
             height: 150px;
             --divider-width: 100%;
             }
          </style>
       </section>
       <section class="section section5 dark" id="section_1140146778">
          <div class="bg section-bg fill bg-fill bg-loaded">
          </div>
          <div class="section-content relative">
             <div class="row row-large" id="row-1636881404">
                <div id="col-62899239" class="col hide-for-small medium-6 small-12 large-6">
                   <div class="col-inner">
                   </div>
                </div>
                <div id="col-1638707039" class="col medium-6 small-12 large-6">
                   <div class="col-inner">
                      <div class="tieu-de">
                         <p>FROM IDEA TO REALIZATION</p>
                         <h2>FROM IDEA TO REALIZATION</h2>
                      </div>
                      <p><span style="color: #ffffff; font-size: 90%;">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur vitae arcu nec metus ultrices scelerisque facilisis id leo. Praesent iaculis lorem ut dictum tempus. Pellentesque sit amet erat at orci bibendum consequat.</span></p>
                      <div class="row row-small" id="row-779800864">
                         <div id="col-257215040" class="col medium-4 small-12 large-4">
                            <div class="col-inner">
                               <div class="icon-box featured-box icon-box-top text-left">
                                  <div class="icon-box-img" style="width: 50px">
                                     <div class="icon">
                                        <div class="icon-inner">
                                           <!--?xml version="1.0" encoding="utf-8"?-->
                                           <!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
                                           <svg width="800px" height="800px" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                              <g id="idea">
                                                 <g id="idea_2">
                                                    <path id="Combined Shape" fill-rule="evenodd" clip-rule="evenodd" d="M23 5V7C23 7.55228 23.4477 8 24 8C24.5523 8 25 7.55228 25 7V5C25 4.44772 24.5523 4 24 4C23.4477 4 23 4.44772 23 5ZM26.887 29.9786H21.123C19.4134 29.9786 18.005 31.3089 18.005 32.9786V41.3486C18.005 42.7823 19.2093 43.9206 20.669 43.9206H27.329C28.7887 43.9206 29.995 42.7823 29.995 41.3486L30.0019 35.5387C34.1757 33.3634 37.0002 28.9948 37.0002 24.0022C37.0002 16.8239 31.1785 11.0022 24.0002 11.0022C16.8206 11.0022 11.0002 16.8232 11.0002 24.0022C11.0002 27.1579 12.1289 30.1397 14.1464 32.4827C14.5068 32.9012 15.1382 32.9484 15.5567 32.588C15.9752 32.2276 16.0224 31.5962 15.662 31.1777C13.9541 29.1942 13.0002 26.6743 13.0002 24.0022C13.0002 17.9278 17.9252 13.0022 24.0002 13.0022C30.0739 13.0022 35.0002 17.9285 35.0002 24.0022C35.0002 27.846 33.0212 31.253 30.0047 33.2223L30.005 32.9818C30.005 31.3093 28.5971 29.9786 26.887 29.9786ZM28.0035 34.2527L28.005 32.9806C28.005 32.4399 27.517 31.9786 26.887 31.9786H21.123C20.4932 31.9786 20.005 32.4398 20.005 32.9786V41.3486C20.005 41.6517 20.2895 41.9206 20.669 41.9206H27.329C27.7092 41.9206 27.995 41.651 27.995 41.3474L28.0009 36.3762C27.0928 36.6692 26.1399 36.8655 25.154 36.9523C24.6038 37.0008 24.1185 36.5941 24.0701 36.044C24.0216 35.4938 24.4283 35.0085 24.9784 34.9601C26.0387 34.8666 27.0538 34.6236 28.0035 34.2527ZM41 23H43C43.5523 23 44 23.4477 44 24C44 24.5523 43.5523 25 43 25H41C40.4477 25 40 24.5523 40 24C40 23.4477 40.4477 23 41 23ZM7 23H5C4.44772 23 4 23.4477 4 24C4 24.5523 4.44772 25 5 25H7C7.55228 25 8 24.5523 8 24C8 23.4477 7.55228 23 7 23ZM10.0001 8.58589L11.4141 9.99989C11.8046 10.3904 11.8046 11.0236 11.4141 11.4141C11.0236 11.8046 10.3904 11.8046 9.99989 11.4141L8.58589 10.0001C8.19537 9.60958 8.19537 8.97642 8.58589 8.58589C8.97642 8.19537 9.60958 8.19537 10.0001 8.58589ZM38.0001 11.4141L39.4141 10.0001C39.8046 9.60958 39.8046 8.97642 39.4141 8.58589C39.0236 8.19537 38.3904 8.19537 37.9999 8.58589L36.5859 9.99989C36.1954 10.3904 36.1954 11.0236 36.5859 11.4141C36.9764 11.8046 37.6096 11.8046 38.0001 11.4141Z" fill="#000000"></path>
                                                 </g>
                                              </g>
                                           </svg>
                                        </div>
                                     </div>
                                  </div>
                                  <div class="icon-box-text last-reset">
                                     <h3>DEVELOPMENT IDEAS</h3>
                                  </div>
                               </div>
                               <div class="icon-box featured-box icon-box-top text-left">
                                  <div class="icon-box-img" style="width: 50px">
                                     <div class="icon">
                                        <div class="icon-inner">
                                           <!--?xml version="1.0" ?-->
                                           <!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
                                           <svg fill="#000000" width="800px" height="800px" viewBox="0 0 64 64" version="1.1" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                              <g id="_x32_5_attachment"></g>
                                              <g id="_x32_4_office"></g>
                                              <g id="_x32_3_pin"></g>
                                              <g id="_x32_2_business_card"></g>
                                              <g id="_x32_1_form">
                                                 <g>
                                                    <g>
                                                       <path d="M54.9351,1H9.0649C7.4746,1,6.1812,2.2939,6.1812,3.8843v8.1963V62c0,0.5522,0.4478,1,1,1h49.6377c0.5522,0,1-0.4478,1-1     V12.0806V3.8843C57.8188,2.2939,56.5254,1,54.9351,1z M8.1812,3.8843C8.1812,3.3965,8.5776,3,9.0649,3h45.8701     c0.4873,0,0.8838,0.3965,0.8838,0.8843v7.1963H8.1812V3.8843z M55.8188,61H8.1812V13.0806h47.6377V61z"></path>
                                                       <path d="M14.9141,33.2871h33.5117c1.1748,0,2.1304-0.9561,2.1304-2.1309v-4.3315c0-1.1768-0.9556-2.1338-2.1304-2.1338H14.9141     c-1.1748,0-2.1304,0.957-2.1304,2.1338v4.3315C12.7837,32.3311,13.7393,33.2871,14.9141,33.2871z M14.7837,26.8247     c0-0.0713,0.061-0.1338,0.1304-0.1338h33.5117c0.0693,0,0.1304,0.0625,0.1304,0.1338v4.3315c0,0.0698-0.061,0.1309-0.1304,0.1309     H14.9141c-0.0693,0-0.1304-0.061-0.1304-0.1309V26.8247z"></path>
                                                       <path d="M14.9141,44.9717h33.5117c1.1748,0,2.1304-0.9556,2.1304-2.1304v-4.335c0-1.1748-0.9556-2.1309-2.1304-2.1309H14.9141     c-1.1748,0-2.1304,0.9561-2.1304,2.1309v4.335C12.7837,44.0161,13.7393,44.9717,14.9141,44.9717z M14.7837,38.5063     c0-0.0684,0.062-0.1309,0.1304-0.1309h33.5117c0.0684,0,0.1304,0.0625,0.1304,0.1309v4.335c0,0.0693-0.061,0.1304-0.1304,0.1304     H14.9141c-0.0693,0-0.1304-0.061-0.1304-0.1304V38.5063z"></path>
                                                       <path d="M22.7197,21.0386h17.8999c0.5522,0,1-0.4478,1-1s-0.4478-1-1-1H22.7197c-0.5522,0-1,0.4478-1,1     S22.1675,21.0386,22.7197,21.0386z"></path>
                                                       <path d="M41.9785,6.0405h-0.0313c-0.5522,0-0.9844,0.4478-0.9844,1s0.4634,1,1.0156,1s1-0.4478,1-1     S42.5308,6.0405,41.9785,6.0405z"></path>
                                                       <path d="M46.4033,6.0405h-0.0313c-0.5522,0-0.9844,0.4478-0.9844,1s0.4634,1,1.0156,1s1-0.4478,1-1     S46.9556,6.0405,46.4033,6.0405z"></path>
                                                       <path d="M50.8281,6.0405h-0.0313c-0.5522,0-0.9844,0.4478-0.9844,1s0.4634,1,1.0156,1s1-0.4478,1-1     S51.3804,6.0405,50.8281,6.0405z"></path>
                                                       <path d="M48.7368,48.292h-8.8086c-1.0029,0-1.8188,0.8159-1.8188,1.8193v3.1318c0,1.0034,0.8159,1.8193,1.8188,1.8193h8.8086     c1.0034,0,1.8193-0.8159,1.8193-1.8193v-3.1318C50.5562,49.1079,49.7402,48.292,48.7368,48.292z M48.5562,53.0625h-8.4468V50.292     h8.4468V53.0625z"></path>
                                                    </g>
                                                 </g>
                                              </g>
                                              <g id="_x32_0_headset"></g>
                                              <g id="_x31_9_video_call"></g>
                                              <g id="_x31_8_letter_box"></g>
                                              <g id="_x31_7_papperplane"></g>
                                              <g id="_x31_6_laptop"></g>
                                              <g id="_x31_5_connection"></g>
                                              <g id="_x31_4_phonebook"></g>
                                              <g id="_x31_3_classic_telephone"></g>
                                              <g id="_x31_2_sending_mail"></g>
                                              <g id="_x31_1_man_talking"></g>
                                              <g id="_x31_0_date"></g>
                                              <g id="_x30_9_review"></g>
                                              <g id="_x30_8_email"></g>
                                              <g id="_x30_7_information"></g>
                                              <g id="_x30_6_phone_talking"></g>
                                              <g id="_x30_5_women_talking"></g>
                                              <g id="_x30_4_calling"></g>
                                              <g id="_x30_3_women"></g>
                                              <g id="_x30_2_writing"></g>
                                              <g id="_x30_1_chatting"></g>
                                           </svg>
                                        </div>
                                     </div>
                                  </div>
                                  <div class="icon-box-text last-reset">
                                     <h3>PROJECTS ADVERTISING</h3>
                                  </div>
                               </div>
                            </div>
                         </div>
                         <div id="col-491326083" class="col medium-4 small-12 large-4">
                            <div class="col-inner">
                               <div class="icon-box featured-box icon-box-top text-left">
                                  <div class="icon-box-img" style="width: 50px">
                                     <div class="icon">
                                        <div class="icon-inner">
                                           <!--?xml version="1.0" encoding="iso-8859-1"?-->
                                           <!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
                                           <svg fill="#000000" height="800px" width="800px" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 384.857 384.857" xml:space="preserve">
                                              <path d="M215.814,151.558c-18.882,0-34.244,15.361-34.244,34.243c0,18.882,15.362,34.243,34.244,34.243s34.243-15.361,34.243-34.243
                                                 C250.057,166.919,234.696,151.558,215.814,151.558z M215.814,202.043c-8.957,0-16.244-7.287-16.244-16.243
                                                 c0-8.956,7.287-16.243,16.244-16.243s16.243,7.287,16.243,16.243C232.057,194.757,224.771,202.043,215.814,202.043z
                                                 M304.122,203.784l-11.629-8.425c0.391-3.164,0.587-6.359,0.587-9.551c0-3.146-0.19-6.291-0.568-9.403l11.632-8.428
                                                 c2.973-1.937,7.028-6.501,5.086-16.565c-0.931-4.817-3.158-10.442-6.272-15.835c-8.63-14.951-19.087-21.493-27.438-17.245
                                                 l-13.091,5.85c-5.099-3.869-10.614-7.063-16.487-9.546l-1.471-14.172c-0.445-9.393-11.339-15.199-28.656-15.199
                                                 c-17.267,0-28.164,5.79-28.654,15.152l-1.476,14.22c-5.874,2.484-11.39,5.679-16.486,9.547l-13.063-5.836
                                                 c-3.164-1.605-9.145-2.834-16.887,3.879c-3.707,3.214-7.464,7.956-10.577,13.35c-8.63,14.945-9.071,27.271-1.221,32.382
                                                 l11.666,8.451c-0.379,3.116-0.569,6.26-0.569,9.399c0,3.191,0.196,6.385,0.587,9.548l-11.671,8.456
                                                 c-7.862,5.105-7.427,17.437,1.207,32.389c3.114,5.395,6.871,10.136,10.578,13.35c7.744,6.714,13.725,5.484,16.887,3.878
                                                 l13.196-5.895c5.057,3.82,10.521,6.978,16.337,9.439l1.491,14.386c0.487,9.365,11.386,15.155,28.654,15.155
                                                 c17.317,0,28.211-5.806,28.656-15.2l1.488-14.341c5.815-2.461,11.28-5.619,16.336-9.438l13.225,5.91
                                                 c8.352,4.251,18.808-2.295,27.437-17.243C311.616,221.205,312.036,208.867,304.122,203.784z M273.929,174.127
                                                 c0.764,3.814,1.15,7.744,1.15,11.681c0,3.971-0.396,7.945-1.175,11.81c-0.697,3.456,0.688,7,3.542,9.068l14.121,10.23
                                                 c-0.412,2.107-1.613,5.805-4.2,10.286c-2.588,4.481-5.189,7.37-6.809,8.78l-15.98-7.142c-3.216-1.438-6.977-0.866-9.62,1.463
                                                 c-5.959,5.248-12.771,9.184-20.248,11.697c-3.342,1.124-5.72,4.095-6.084,7.602l-1.807,17.407
                                                 c-2.031,0.696-5.833,1.505-11.006,1.505c-5.176,0-8.979-0.809-11.009-1.506l-1.805-17.405c-0.364-3.507-2.742-6.479-6.084-7.603
                                                 c-7.476-2.513-14.288-6.449-20.25-11.698c-2.643-2.328-6.399-2.9-9.618-1.463l-15.983,7.14c-1.618-1.41-4.22-4.299-6.806-8.779
                                                 c-2.588-4.482-3.789-8.18-4.2-10.287l14.12-10.231c2.854-2.068,4.238-5.611,3.542-9.066c-0.778-3.866-1.174-7.839-1.174-11.809
                                                 c0-3.924,0.388-7.855,1.151-11.684c0.688-3.45-0.697-6.985-3.546-9.049l-14.094-10.209c0.411-2.107,1.612-5.806,4.201-10.289
                                                 c2.585-4.479,5.187-7.368,6.805-8.778l15.862,7.087c3.226,1.44,6.992,0.862,9.635-1.477c5.987-5.298,12.84-9.267,20.368-11.795
                                                 c3.343-1.123,5.722-4.095,6.086-7.603l1.789-17.239c2.03-0.697,5.833-1.506,11.009-1.506c5.173,0,8.975,0.808,11.006,1.505
                                                 l1.789,17.24c0.364,3.508,2.744,6.48,6.088,7.603c7.525,2.526,14.378,6.494,20.369,11.794c2.644,2.339,6.41,2.917,9.635,1.476
                                                 l15.859-7.087c1.618,1.41,4.221,4.299,6.808,8.781c2.587,4.479,3.788,8.176,4.2,10.284l-14.094,10.212
                                                 C274.624,167.139,273.239,170.676,273.929,174.127z M321.874,46h-212.12c-34.221,0-61.825,27.754-61.825,61.976v175.411l-29,25.801
                                                 V105.811C18.928,57.195,58.973,18,107.59,18h168.009c9.445,0,18.467,2.804,26.09,8.446c3.995,2.957,9.631,2.026,12.589-1.969
                                                 c2.957-3.995,2.115-9.497-1.88-12.454C301.652,4.07,288.927,0,275.598,0H107.59C49.049,0,0.928,47.27,0.928,105.811v223.776
                                                 c0,3.565,2.351,6.795,5.614,8.234c3.262,1.438,7.308,0.817,9.942-1.587l31.445-28.478v68.101c0,3.565,1.986,6.795,5.249,8.234
                                                 c1.167,0.515,2.344,0.766,3.572,0.766c2.203,0,4.348-0.809,6.04-2.352l52.297-47.742c1.627-1.485,3.009-2.782,4.222-3.913
                                                 c2.503-2.336,4.864-4.542,5.647-4.849c0.735-0.289,3.838-0.171,7.122-0.128c1.777,0.024,3.838,0.127,6.266,0.127h183.531
                                                 c34.221,0,62.055-27.993,62.055-62.215V107.976C383.928,73.754,356.094,46,321.874,46z M365.928,263.785
                                                 c0,24.297-19.759,44.215-44.055,44.215H138.343c-2.334,0-4.317-0.103-6.026-0.125c-13.266-0.178-15.396,0.617-25.296,9.852
                                                 c-1.171,1.093-2.396,2.327-3.967,3.761l-37.125,33.97v-68.091c0-0.008,0-0.015,0-0.023V107.976C65.928,83.679,85.458,64,109.754,64
                                                 h212.12c24.296,0,44.055,19.679,44.055,43.976V263.785z"></path>
                                           </svg>
                                        </div>
                                     </div>
                                  </div>
                                  <div class="icon-box-text last-reset">
                                     <h3>TECHNICAL DEVELOPMENT</h3>
                                  </div>
                               </div>
                               <div class="icon-box featured-box icon-box-top text-left">
                                  <div class="icon-box-img" style="width: 50px">
                                     <div class="icon">
                                        <div class="icon-inner">
                                           <!--?xml version="1.0" ?-->
                                           <!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
                                           <svg fill="#000000" width="800px" height="800px" viewBox="0 0 64 64" version="1.1" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                              <g id="_x32_5_attachment"></g>
                                              <g id="_x32_4_office"></g>
                                              <g id="_x32_3_pin"></g>
                                              <g id="_x32_2_business_card"></g>
                                              <g id="_x32_1_form"></g>
                                              <g id="_x32_0_headset"></g>
                                              <g id="_x31_9_video_call"></g>
                                              <g id="_x31_8_letter_box"></g>
                                              <g id="_x31_7_papperplane">
                                                 <g>
                                                    <g>
                                                       <path d="M62.9891,2.5618c-0.0765-0.5779-0.6611-0.9805-1.2299-0.8401L7.4043,15.2065c-0.3535,0.0879-0.6318,0.3608-0.7256,0.7129     s0.0112,0.7275,0.2744,0.9795l13.9343,13.3583l-2.7649,17.1495c-0.1079,0.6712,0.4969,1.2576,1.1582,1.1445l18.0805-3.1324     l17.1832,9.6988c0.1523,0.0859,0.3218,0.1289,0.4917,0.1289c0.1523,0,0.3047-0.0347,0.4453-0.1045     c0.2969-0.1475,0.5015-0.4331,0.5459-0.7617l6.9639-51.5542C63.0031,2.7372,63.0007,2.6487,62.9891,2.5618z M9.647,16.7109     L56.8914,4.9902L22.2545,28.7978L9.647,16.7109z M36.9146,43.4663l-16.5942,2.875l2.4995-15.5054L58.8633,6.0615L36.9146,43.4663     z M54.2427,52.6504l-15.3231-8.6492l21.4231-36.509L54.2427,52.6504z"></path>
                                                       <path d="M14.4438,51.6099l-4.6948,5.209c-0.3701,0.4102-0.3369,1.0425,0.0732,1.4121c0.1909,0.1724,0.4307,0.2573,0.6689,0.2573     c0.2734,0,0.5459-0.1113,0.7432-0.3306l4.6948-5.209c0.3701-0.4102,0.3369-1.0425-0.0732-1.4121     C15.4463,51.1675,14.8135,51.2002,14.4438,51.6099z"></path>
                                                       <path d="M5.9478,29.0562l-4.6909,5.2085c-0.3696,0.4106-0.3364,1.043,0.0742,1.4126c0.1909,0.1719,0.4302,0.2568,0.6685,0.2568     c0.2739,0,0.5459-0.1113,0.7437-0.3311l4.6909-5.2085c0.3696-0.4106,0.3364-1.043-0.0742-1.4126     C6.9487,28.6128,6.3179,28.6455,5.9478,29.0562z"></path>
                                                       <path d="M40.8164,55.4331l-4.6909,5.2051c-0.3701,0.4102-0.3369,1.0425,0.0732,1.4121c0.1909,0.1724,0.4307,0.2573,0.6689,0.2573     c0.2734,0,0.5459-0.1113,0.7432-0.3306l4.6909-5.2051c0.3701-0.4102,0.3369-1.0425-0.0732-1.4121     C41.8188,54.9907,41.186,55.0234,40.8164,55.4331z"></path>
                                                    </g>
                                                 </g>
                                              </g>
                                              <g id="_x31_6_laptop"></g>
                                              <g id="_x31_5_connection"></g>
                                              <g id="_x31_4_phonebook"></g>
                                              <g id="_x31_3_classic_telephone"></g>
                                              <g id="_x31_2_sending_mail"></g>
                                              <g id="_x31_1_man_talking"></g>
                                              <g id="_x31_0_date"></g>
                                              <g id="_x30_9_review"></g>
                                              <g id="_x30_8_email"></g>
                                              <g id="_x30_7_information"></g>
                                              <g id="_x30_6_phone_talking"></g>
                                              <g id="_x30_5_women_talking"></g>
                                              <g id="_x30_4_calling"></g>
                                              <g id="_x30_3_women"></g>
                                              <g id="_x30_2_writing"></g>
                                              <g id="_x30_1_chatting"></g>
                                           </svg>
                                        </div>
                                     </div>
                                  </div>
                                  <div class="icon-box-text last-reset">
                                     <h3>PROJECTS INSTALLATION</h3>
                                  </div>
                               </div>
                            </div>
                         </div>
                         <div id="col-749383612" class="col medium-4 small-12 large-4">
                            <div class="col-inner">
                               <div class="icon-box featured-box icon-box-top text-left">
                                  <div class="icon-box-img" style="width: 50px">
                                     <div class="icon">
                                        <div class="icon-inner">
                                           <!--?xml version="1.0" encoding="iso-8859-1"?-->
                                           <!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
                                           <svg fill="#000000" height="800px" width="800px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512" xml:space="preserve">
                                              <g>
                                                 <g>
                                                    <g>
                                                       <path d="M370.164,257.081H252.131c-6.147,0-11.13,4.984-11.13,11.13c0,6.146,4.983,11.13,11.13,11.13h118.032
                                                          c6.147,0,11.13-4.984,11.13-11.13C381.293,262.066,376.311,257.081,370.164,257.081z"></path>
                                                       <path d="M249.002,180.744l5.802-6.448v49.339c0,6.146,4.983,11.13,11.13,11.13c6.147,0,11.13-4.984,11.13-11.13v-49.339
                                                          l5.802,6.448c2.198,2.442,5.231,3.685,8.278,3.685c2.652,0,5.315-0.943,7.442-2.857c4.569-4.112,4.94-11.149,0.828-15.718
                                                          l-25.206-28.008c-2.11-2.345-5.118-3.685-8.273-3.685c-3.156,0-6.163,1.339-8.273,3.685l-25.206,28.008
                                                          c-4.112,4.569-3.742,11.607,0.828,15.718C237.853,185.685,244.891,185.313,249.002,180.744z"></path>
                                                       <path d="M364.633,137.845c-2.11-2.345-5.118-3.685-8.273-3.685c-3.156,0-6.163,1.339-8.273,3.685l-25.206,28.008
                                                          c-4.112,4.569-3.742,11.607,0.828,15.718c4.569,4.114,11.607,3.742,15.719-0.827l5.802-6.448v49.339
                                                          c0,6.146,4.983,11.13,11.13,11.13c6.147,0,11.13-4.984,11.13-11.13v-49.339l5.802,6.448c2.198,2.442,5.231,3.685,8.278,3.685
                                                          c2.652,0,5.315-0.943,7.442-2.857c4.569-4.112,4.94-11.149,0.828-15.718L364.633,137.845z"></path>
                                                       <path d="M500.87,372.34v-0.001c6.147,0,11.13-4.984,11.13-11.13v-44.522c0-6.146-4.983-11.13-11.13-11.13h-63.458V39.486
                                                          c0-6.146-4.983-11.13-11.13-11.13H85.719c-6.147,0-11.13,4.984-11.13,11.13v266.072H11.13c-6.147,0-11.13,4.984-11.13,11.13
                                                          v44.522c0,6.146,4.983,11.13,11.13,11.13h38.957v44.522H11.13c-6.147,0-11.13,4.984-11.13,11.13v44.522
                                                          c0,6.146,4.983,11.13,11.13,11.13H500.87c6.147,0,11.13-4.984,11.13-11.13v-44.522c0-6.146-4.983-11.13-11.13-11.13h-38.957
                                                          V372.34H500.87z M222.609,50.616h66.783v24.521l-5.402-3.24c-3.525-2.117-7.929-2.114-11.454-0.001l-16.533,9.921l-16.537-9.921
                                                          c-1.762-1.057-3.744-1.586-5.727-1.586s-3.965,0.529-5.727,1.586l-5.404,3.243V50.616z M96.849,101.127h28.925
                                                          c6.147,0,11.13-4.984,11.13-11.13s-4.983-11.13-11.13-11.13H96.849v-28.25h103.499v44.181c0,4.01,2.157,7.71,5.646,9.686
                                                          s7.772,1.922,11.211-0.141l16.536-9.921l16.536,9.921c1.762,1.057,3.744,1.586,5.727,1.586c1.982,0,3.965-0.53,5.727-1.586
                                                          l16.533-9.921l16.532,9.919c3.439,2.065,7.722,2.118,11.211,0.143c3.489-1.976,5.646-5.677,5.646-9.686V50.616h103.499v254.941
                                                          H96.849V101.127z M489.739,439.122v22.261H22.261v-22.261H489.739z M72.348,416.861v-44.522h33.391v44.522H72.348z M128,416.861
                                                          v-44.522h89.043v44.522H128z M239.304,416.861v-44.522h33.391v44.522H239.304z M294.957,416.861v-44.522H384v44.522H294.957z
                                                          M406.261,416.861v-44.522h33.391v44.522H406.261z M22.261,350.078v-22.261h467.478v22.261H22.261z"></path>
                                                       <path d="M166.036,78.866h-1.113c-6.147,0-11.13,4.984-11.13,11.13s4.983,11.13,11.13,11.13h1.113
                                                          c6.147,0,11.13-4.984,11.13-11.13S172.183,78.866,166.036,78.866z"></path>
                                                    </g>
                                                 </g>
                                              </g>
                                           </svg>
                                        </div>
                                     </div>
                                  </div>
                                  <div class="icon-box-text last-reset">
                                     <h3>REALIZATION OF PROJECTS</h3>
                                  </div>
                               </div>
                               <div class="icon-box featured-box icon-box-top text-left">
                                  <div class="icon-box-img" style="width: 50px">
                                     <div class="icon">
                                        <div class="icon-inner">
                                           <!--?xml version="1.0" ?-->
                                           <!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
                                           <svg fill="#000000" width="800px" height="800px" viewBox="0 0 64 64" version="1.1" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                              <g id="_x32_5_attachment"></g>
                                              <g id="_x32_4_office"></g>
                                              <g id="_x32_3_pin"></g>
                                              <g id="_x32_2_business_card"></g>
                                              <g id="_x32_1_form"></g>
                                              <g id="_x32_0_headset"></g>
                                              <g id="_x31_9_video_call"></g>
                                              <g id="_x31_8_letter_box"></g>
                                              <g id="_x31_7_papperplane"></g>
                                              <g id="_x31_6_laptop"></g>
                                              <g id="_x31_5_connection"></g>
                                              <g id="_x31_4_phonebook">
                                                 <g>
                                                    <g>
                                                       <path d="M32.4961,46.3589c1.6934,1.1709,4.3477,2.3223,7.042,2.3223c0.3174,0,0.6348-0.0161,0.9512-0.0498     c1.6719-0.1797,3.3721-0.8574,4.5537-1.8174c0.0176-0.0146,1.7832-1.4858,1.8027-2.9702     c0.0215-1.6548-1.4844-3.2998-3.0098-4.8228c-2.3818-2.5146-3.9814-3.4844-6.623-2.2104     c-0.0605,0.0293-0.1172,0.0645-0.1709,0.105c-0.5088-0.0005-2.2656-1.0283-3.7383-3.2432     c-0.3477-0.52-0.6885-1.0938-1.0117-1.7036c-0.3203-0.6074-0.6025-1.2119-0.8379-1.7969c-1.043-2.585-0.834-4.5225-0.5654-4.918     c0.0625-0.021,0.123-0.0483,0.1807-0.0811c2.5498-1.4648,2.6475-3.333,1.917-6.689c-0.4023-2.1445-0.9111-4.3154-2.2881-5.2319     c-1.0342-0.6855-2.8604-0.3662-3.4756-0.1841c-1.4336,0.4248-2.9902,1.4727-4.0605,2.7334     c-1.9629,2.3037-2.6484,5.5825-2.6348,7.8813c0.0176,3.7134,1.3633,8.4219,3.5986,12.5957     C26.3105,40.4785,29.4385,44.2466,32.4961,46.3589z M24.6846,17.0972c0.8223-0.9673,2.041-1.7959,3.1055-2.1118     c0.3594-0.106,0.8105-0.1553,1.1777-0.1553c0.2891,0,0.5254,0.0308,0.624,0.0879c0.7891,0.5254,1.2373,2.9146,1.4346,3.9629     c0.6934,3.1865,0.3467,3.791-0.8545,4.5c-0.3428,0.1182-0.7578,0.3838-1.0664,0.9722c-0.7295,1.3887-0.5313,4.0278,0.4932,6.5669     c0.2617,0.6479,0.5723,1.3145,0.9258,1.9839c0.3555,0.6714,0.7314,1.3037,1.1143,1.8774     c1.5176,2.2808,3.585,3.9346,5.1445,4.1152c0.6553,0.0781,1.1123-0.1162,1.4033-0.3345c1.2822-0.603,1.9434-0.5459,4.2168,1.854     c0.7344,0.7339,2.4551,2.4526,2.4424,3.3979c-0.0186,0.2598-0.5537,1.0273-1.0693,1.4521c-0.8623,0.7007-2.2363,1.2407-3.5,1.377     c-2.46,0.2627-5.1191-0.8755-6.6436-1.9297c-2.8027-1.9365-5.6943-5.4365-7.7344-9.3628     c-0.002-0.0039-0.0039-0.0078-0.0059-0.0112c-2.0908-3.9014-3.3496-8.2622-3.3652-11.6675     C22.5166,21.8198,23.0752,18.9873,24.6846,17.0972z"></path>
                                                       <path d="M59.1758,10.936h-2.7031V7.7515C56.4727,4.0288,53.4443,1,49.7217,1H12.4756c-3.7227,0-6.751,3.0288-6.751,6.7515v5.708     H5.4932c-2.1973,0-3.9844,1.7861-3.9844,3.9814s1.7871,3.9814,3.9844,3.9814h0.2314v6.5942H5.2539     c-2.1953,0-3.9805,1.7861-3.9805,3.9814c0,2.1978,1.7852,3.9854,3.9805,3.9854h0.4707v6.9683H5.2471     c-2.1953,0-3.9814,1.7861-3.9814,3.9814s1.7861,3.9814,3.9814,3.9814h0.4775v5.334c0,3.7227,3.0283,6.7515,6.751,6.7515h37.2461     c3.7227,0,6.751-3.0288,6.751-6.7515v-1.9639h2.7031c1.9619,0,3.5586-1.5947,3.5586-3.5552V14.4907     C62.7344,12.5308,61.1377,10.936,59.1758,10.936z M56.4727,29.0879h4.2617v6.3584h-4.2617V29.0879z M56.4727,27.0879v-5.8872     h4.2617v5.8872H56.4727z M56.4727,37.4463h4.2617v6.355h-4.2617V37.4463z M59.1758,12.936c0.8594,0,1.5586,0.6973,1.5586,1.5547     v4.71h-4.2617V12.936H59.1758z M12.4756,3h37.2461c2.2766,0,4.1798,1.6108,4.6407,3.7515H7.8348     C8.2958,4.6108,10.199,3,12.4756,3z M3.5088,17.4409c0-1.0928,0.8906-1.9814,1.9844-1.9814h2.9404     c1.0928,0,1.9814,0.8887,1.9814,1.9814s-0.8887,1.9814-1.9814,1.9814H5.4932C4.3994,19.4224,3.5088,18.5337,3.5088,17.4409z      M3.2734,31.998c0-1.0928,0.8887-1.9814,1.9805-1.9814h2.9404c1.0928,0,1.9814,0.8887,1.9814,1.9814     c0,1.0947-0.8887,1.9854-1.9814,1.9854H5.2539C4.1621,33.9834,3.2734,33.0928,3.2734,31.998z M3.2656,46.9331     c0-1.0928,0.8887-1.9814,1.9814-1.9814h2.9404c1.0928,0,1.9815,0.8887,1.9815,1.9814s-0.8887,1.9814-1.9815,1.9814H5.2471     C4.1543,48.9146,3.2656,48.0259,3.2656,46.9331z M8.1875,50.9146c2.1953,0,3.9815-1.7861,3.9815-3.9814     s-1.7861-3.9814-3.9815-3.9814H7.7246v-6.9683h0.4697c2.1953,0,3.9814-1.7876,3.9814-3.9854     c0-2.1953-1.7861-3.9814-3.9814-3.9814H7.7246v-6.5942h0.709c2.1953,0,3.9814-1.7861,3.9814-3.9814s-1.7861-3.9814-3.9814-3.9814     h-0.709v-4.708h46.748v3.1846v41.3486v1.9639H7.7246v-4.334H8.1875z M49.7217,61H12.4756c-2.2766,0-4.1798-1.6108-4.6407-3.7515     h46.5276C53.9015,59.3892,51.9983,61,49.7217,61z M59.1758,52.2847h-2.7031v-6.4834h4.2617v4.9282     C60.7344,51.5869,60.0352,52.2847,59.1758,52.2847z"></path>
                                                    </g>
                                                 </g>
                                              </g>
                                              <g id="_x31_3_classic_telephone"></g>
                                              <g id="_x31_2_sending_mail"></g>
                                              <g id="_x31_1_man_talking"></g>
                                              <g id="_x31_0_date"></g>
                                              <g id="_x30_9_review"></g>
                                              <g id="_x30_8_email"></g>
                                              <g id="_x30_7_information"></g>
                                              <g id="_x30_6_phone_talking"></g>
                                              <g id="_x30_5_women_talking"></g>
                                              <g id="_x30_4_calling"></g>
                                              <g id="_x30_3_women"></g>
                                              <g id="_x30_2_writing"></g>
                                              <g id="_x30_1_chatting"></g>
                                           </svg>
                                        </div>
                                     </div>
                                  </div>
                                  <div class="icon-box-text last-reset">
                                     <h3>TECHNICAL SUPPORT</h3>
                                  </div>
                               </div>
                            </div>
                         </div>
                      </div>
                   </div>
                </div>
             </div>
          </div>
          <style>
             #section_1140146778 {
             padding-top: 100px;
             padding-bottom: 100px;
             background-color: rgb(51, 51, 51);
             }
             #section_1140146778 .section-bg.bg-loaded {
             background-image: url(https://solar3.maugiaodien.com/wp-content/uploads/2023/01/services.jpg);
             }
             #section_1140146778 .ux-shape-divider--top svg {
             height: 150px;
             --divider-top-width: 100%;
             }
             #section_1140146778 .ux-shape-divider--bottom svg {
             height: 150px;
             --divider-width: 100%;
             }
          </style>
       </section>
       <section class="section gioi-thieu-2" id="section_743354186">
          <div class="bg section-bg fill bg-fill bg-loaded">
          </div>
          <div class="section-content relative">
             <div class="row" id="row-1379167350">
                <div id="col-2061728723" class="col medium-7 small-12 large-7">
                   <div class="col-inner">
                      <div class="row row-small" id="row-811246866">
                         <div id="col-2005530496" class="col medium-6 small-12 large-6">
                            <div class="col-inner">
                               <div class="img has-hover x md-x lg-x y md-y lg-y" id="image_1994958208">
                                  <div class="img-inner dark">
                                     <img decoding="async" width="250" height="100" src="https://solar3.maugiaodien.com/wp-content/uploads/2023/01/1-1.png" class="attachment-original size-original" alt="">						
                                  </div>
                                  <style>
                                     #image_1994958208 {
                                     width: 100%;
                                     }
                                  </style>
                               </div>
                               <div class="img has-hover x md-x lg-x y md-y lg-y" id="image_1634903619">
                                  <div class="img-inner dark">
                                     <img loading="lazy" decoding="async" width="250" height="100" src="https://solar3.maugiaodien.com/wp-content/uploads/2023/01/3-1.png" class="attachment-original size-original" alt="">						
                                  </div>
                                  <style>
                                     #image_1634903619 {
                                     width: 100%;
                                     }
                                  </style>
                               </div>
                            </div>
                         </div>
                         <div id="col-2119359794" class="col medium-6 small-12 large-6">
                            <div class="col-inner">
                               <div class="img has-hover x md-x lg-x y md-y lg-y" id="image_1161239726">
                                  <div class="img-inner dark">
                                     <img loading="lazy" decoding="async" width="250" height="100" src="https://solar3.maugiaodien.com/wp-content/uploads/2023/01/2-1.png" class="attachment-original size-original" alt="">						
                                  </div>
                                  <style>
                                     #image_1161239726 {
                                     width: 100%;
                                     }
                                  </style>
                               </div>
                               <div class="img has-hover x md-x lg-x y md-y lg-y" id="image_901872829">
                                  <div class="img-inner dark">
                                     <img loading="lazy" decoding="async" width="250" height="100" src="https://solar3.maugiaodien.com/wp-content/uploads/2023/01/4-1.png" class="attachment-original size-original" alt="">						
                                  </div>
                                  <style>
                                     #image_901872829 {
                                     width: 100%;
                                     }
                                  </style>
                               </div>
                            </div>
                         </div>
                      </div>
                   </div>
                </div>
                <div id="col-253859050" class="col medium-5 small-12 large-5">
                   <div class="col-inner">
                      <p>MAJOR AND BEST</p>
                      <h2><span style="font-size: 130%;">OUR CLIENTS</span></h2>
                      <p><span style="font-size: 95%;">Cras feugiat nibh vel imperdiet porta. Nam ut libero scelerisque, porta ligula in, mattis neque. Duis consequat, eros eu mollis venenatis, lectus nisi sagittis lorem, a lacinia arcu lectus vitae ante. Sed tempus aliquam erat, ac euismod erat commodo vel. Etiam semper massa at lectus congue tristiq. Lorem ipsum dolor sit amet, consectetur adipiscing elit.</span></p>
                      <p><em><strong><span style="font-size: 95%;">Sed ut maximus neque. Quisque lectus tortor, pellente vitae fringilla et, commodo eu orci. Praesent aliquet, dui eu varius pellentesque, magna quam feugiat ex.</span></strong></em></p>
                   </div>
                </div>
             </div>
          </div>
          <style>
             #section_743354186 {
             padding-top: 60px;
             padding-bottom: 60px;
             }
             #section_743354186 .ux-shape-divider--top svg {
             height: 150px;
             --divider-top-width: 100%;
             }
             #section_743354186 .ux-shape-divider--bottom svg {
             height: 150px;
             --divider-width: 100%;
             }
          </style>
       </section>
    </div>



        {{-- include('layouts.layout102.landingpage.content') --}}

        @include('common.modal_landingpage_edit')

        @include('common.modal_sort_order')


@endsection