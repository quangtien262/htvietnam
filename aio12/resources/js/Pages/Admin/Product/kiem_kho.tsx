...
export default function Dashboard(props) {
    ...
    const onFinishFormEdit = (values: any) => {
        ...
        // save
        axios.post(route("saveKiemKho"), values).then((response) => {
    ...
    const expandedRowRender = (record, index) => {
        ...
        const detail = info.map((item, idx) => {
            return <tr key={idx}>
                ...
                <td>{item.ton_kho}</td>
                <td>{item.thuc_te}</td>
                <td>{item.so_luong_lech}</td>
                <td>{numberFormat(item.gia_tri_lech)}</td>
            </tr>
        });
            ...
            <table className="table-sub">
                ...
                <tbody>
                    <tr className="border-none">
                        <td colSpan={3} className="text-right01">Tổng lệch tăng:</td>
                        <td>{record.so_luong_lech_tang}</td>
                        <td>{numberFormat(record.tong_tien_lech_tang)}</td>
                    </tr>
                    <tr className="border-none">
                        <td>
                            <a target="new" href={route('print.kiemKho', [record.id])}>
                                <button className="btn-print"><PrinterOutlined /> In</button>
                            </a>
                        </td>
                        <td colSpan={2} className="text-right01">Tổng lệch giảm:</td>
                        <td>{record.so_luong_lech_giam}</td>
                        <td>{numberFormat(record.tong_tien_lech_giam)}</td>
                    </tr>
                    <tr className="border-none">
                        <td>
    ...
    function showDataDetail() {
        return dataDetail.map((data, idx) => {
            return <tr key={idx}>
                ...
                <td className="td-input">
                    {numberFormat(data.gia_tri_lech)}
                </td>
                <td>
    ...
    function showTotalDetail() {
        ...
        return <tbody>
            <tr className="border-none">
                <td colSpan={3} className="text-right01">Tổng lệch tăng:</td>
                <td>{slTang}</td>
                <td>{numberFormat(tienLechTang)}</td>
            </tr>
            <tr className="border-none">
                <td colSpan={3} className="text-right01">Tổng lệch giảm:</td>
                <td>{slGiam}</td>
                <td>{numberFormat(tienLechGiam)}</td>
            </tr>
            <tr className="border-none">
