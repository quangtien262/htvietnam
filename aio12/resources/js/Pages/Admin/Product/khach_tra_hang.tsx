...
export default function Dashboard(props) {
    ...
    const [tongTraKhach, setTongTraKhach] = useState(0);
    ...
    const onFinishSearch = (values) => {
        ...
        router.get(route("khachTraHang"),values);
    };

    function addSub() {
        let data_tmp = cloneDeep(dataDetail);
        data_tmp.push(dataDetail_item_default);
        setDataDetail(data_tmp);
        setStateTotal(data_tmp);
    }
    ...
    const onFinishFormEdit = (values) => {
        ...
        values.tongTraKhach = tongTraKhach;
        values.tongSoLuong = tongSoLuong;
        values.tongGiamGia = tongGiamGia;

        values.daThanhToan = daThanhToan;
        values.tongCongNo = tongCongNo;
        values.ngayTatToan = ngayTatToan.format('YYYY-MM-DD');
        ...
        // save
        axios.post(route("saveKhachTraHang"), values).then((response) => {
        ...
        return <div>
            ...
            <table className="table-sub">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <button className="btn-cancel02" onClick={() => {setIsModalXoaOpen(true); setIdAction(record.id)}}>
                ...
                <td>
                    <Select className="select03"
                        ...
                        onChange={(value, info) => {
                            ...
                            data_tmp[idx].giam_gia = 0;
                            data_tmp[idx].tien_tra_khach = info.gia_ban;
                            setDataDetail(data_tmp);
                            setStateTotal(data_tmp);
                        }}
                        allowClear={true}
                ...
                <td className="td-input">
                        ...
                        <InputNumber 
                            ...
                            onChange={(value) => {
                                ...
                                data_tmp[idx].tien_tra_khach = value * dataDetail[idx].gia_ban - dataDetail[idx].giam_gia;
                                setDataDetail(data_tmp);
                                setStateTotal(data_tmp);
                    ...
                    <Popconfirm 
                        ...
                        <div>
                            <InputNumber 
                                ...
                                onChange={(value) => {
                                    let data_tmp = cloneDeep(dataDetail);
                                    let traKhach =  dataDetail[idx].so_luong * dataDetail[idx].gia_ban;
                                    data_tmp[idx].giam_gia = value;

                                    if(data.is_percen) {
                                        data_tmp[idx].tien_tra_khach = traKhach - (value * traKhach / 100);
                                    } else {
                                        data_tmp[idx].tien_tra_khach = traKhach - value;
                                    }
                            ...
                            />
                            <Switch value={data.is_percen} 
                                checkedChildren="%"
                                unCheckedChildren="Đ"
                                onChange={(value) => {
                                    let data_tmp = cloneDeep(dataDetail);
                                    data_tmp[idx].is_percen = value;
                                    data_tmp[idx].giam_gia = 0;
                                    data_tmp[idx].don_vi_giam_gia = value ? '%':'Đ';
                                    setDataDetail(data_tmp);
                                    setStateTotal(data_tmp);
                                }}
                            />
                        ...
                        <a>{data.giam_gia} {data.is_percen ? '%':<sup>đ</sup>}</a>
                    </Popconfirm>
                    
                </td>
                <td className="td-input">
                    {numberFormat(data.tien_tra_khach)} <sup>đ</sup>
                </td>
                <td>
    ...
    function setStateTotal(detail) {
        let traKhach = 0;
        let giamGia = 0;
        let soLuong = 0;
        detail.forEach((val) => {
            if(val.is_percen) {
                giamGia += (val.giam_gia * val.tien_tra_khach)/100;
            } else {
                giamGia += val.giam_gia;
            }
            traKhach += val.tien_tra_khach;
            soLuong += val.so_luong;
        });
        setTongSoLuong(soLuong);
        setTongGiamGia(giamGia);
        setTongTraKhach(traKhach);
        setDaThanhToan(traKhach);
        setTongCongNo(0);
    ...
    function showTotalDetail() {
        return <tbody>
            ...
            <tr>
                <td colSpan={2} className="text-right01 ">Tổng trả khách:</td>
                <td className="_bold">{numberFormat(tongSoLuong)} </td>
                <td className="_bold">{numberFormat(tongGiamGia)} <sup>đ</sup></td>
                <td className="_bold">{numberFormat(tongTraKhach)} <sup>đ</sup></td>
            </tr>

            {/* Đã thanh toán */}
            <tr>
                <td colSpan={2} className="text-right01 ">Đã thanh toán:</td>
                <td colSpan={2}></td>
                <td className="_bold">
                    <InputNumber value={daThanhToan} className="width100"
                        ...
                        onChange={(value) => {
                            ...
                            if(value < tongTraKhach) {
                                setTongCongNo(tongTraKhach - value);
                                setNgayTatToan(null);
                            } else {
                                setTongCongNo(0);
                                setNgayTatToan(dayjs());
            ...
            {/* Công nợ */}
            <tr>
                <td colSpan={2} className="text-right01 ">
                    <b><a><MinusCircleOutlined /> </a> </b>
                    <b>Công nợ </b>
                </td>
                <td colSpan={2}></td>
            ...
            <tr className="border-none">
                <td colSpan={6}>
                    ...
                    <p><b className="success-text">Chọn phương thức thanh toán: </b></p>
                    <Form.Item
                        label=''
                ...
                content={
                    <div>
                        <Modal title="Xác nhận xóa" 
                            open={isModalXoaOpen} 
                            onOk={async () => {
                                    const result = await callApi(route('hoa_don.huyHoaDon.khachTraHang', [idAction]));
                        ...
                        <Modal title="THANH TOÁN CÔNG NỢ" 
                            ...
                            onOk={() => {
                                    ...
                                    axios.post(route("tatToanCongNo"), {
                                                loai_chung_tu: 'product_khach_tra_hang',
