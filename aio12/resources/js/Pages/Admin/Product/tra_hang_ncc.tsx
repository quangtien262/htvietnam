...
export default function Dashboard(props) {
    ...
    const [phiTraHang, setPhiTraHang] = useState(0);
    ...
    const onFinishFormEdit = (values) => {
        ...
        values.giam_gia = giamGia;
        values.phi_tra_hang = phiTraHang;
        ...
        // save
        axios.post(route("saveTraHangNCC"), values).then((response) => {
    ...
    const onFinishSearch = (values) => {
        ...
        router.get(route("traHangNCC"),values);
    };
            ...
            return <tr key={idx}>
                <td>
                    <Select className="select03"
                        ...
                        onChange={(value, info) => {
                            ...
                            data_tmp[idx].gia_tra_lai = info.gia_von;
                ...
                <td className="td-input">
                    <InputNumber 
                        ...
                        onChange={(value) => {
                            ...
                            data_tmp[idx].thanh_tien = value * dataDetail[idx].gia_tra_lai;
                            setDataDetail(data_tmp);
                            tinhTongTien(data_tmp);
                        }}
                    />
                ...
                <td className="td-input">
                    <InputNumber 
                        ...
                        parser={(value) => value.replace(/\$\s?|(,*)/g, "")}
                        disabled={data.product_id ? false : true}
                        value={data.gia_tra_lai}
                        min={0}
                        onChange={(value) => {
                            let data_tmp = cloneDeep(dataDetail);
                            data_tmp[idx].gia_tra_lai = value;
                            data_tmp[idx].thanh_tien = value * dataDetail[idx].so_luong;
    ...
    function tinhTongTien(dataDetail_tmp) {
        ...
        dataDetail_tmp.forEach((val) => {
            tongTienHang_tmp += val.thanh_tien;
        });
        const thanhToan_tmp = tongTienHang_tmp + phiTraHang - giamGia;
        setThanhToan(thanhToan_tmp);
        setTongTienHang(tongTienHang_tmp);
        
        setDaThanhToan(thanhToan_tmp);
        setTongCongNo(0);
    ...
    function showTotalDetail() {
        ...
        return <tbody>
            <tr>
                <td colSpan={4} className="text-right01">Tổng tiền hàng:</td>
                <td className="_red">{numberFormat(tongTienHang)} <sup>đ</sup></td>
            </tr>
            <tr>
                <td colSpan={4} className="text-right01">Giảm giá:</td>
                <td className="_red">
                    <InputNumber
                        min={0}
                        style={{ width: "150px" }}
                        ...
                        value={giamGia}
                        onChange={(value) => {
                            const thanhToan_tmp = tongTienHang + phiTraHang - value;
                            setThanhToan(thanhToan_tmp);
                            setGiamGia(value);
                        }}
                    />
            ...
            <tr>
                <td colSpan={4} className="text-right01">Phí trả hàng:</td>
                <td className="_red">
                    <InputNumber
                        min={0}
                        style={{ width: "150px" }}
                        ...
                        value={phiTraHang}
                        onChange={(value) => {
                            const thanhToan_tmp = tongTienHang + value - giamGia;
                            setThanhToan(thanhToan_tmp);
                            setPhiTraHang(value);
                        }}
                    />
            ...
            <tr>
                <td colSpan={4} className="text-right01">Thanh Toán:</td>
                <td className="_red">{numberFormat(thanhToan)} <sup>đ</sup></td>
            </tr>

            {/* Đã thanh toán */}
            <tr>
                <td colSpan={4} className="text-right01 ">Đã thanh toán:</td>
                <td className="_bold">
            ...
            {/* Công nợ */}
            <tr>
                <td colSpan={4} className="text-right01 ">
                    <b><a><MinusCircleOutlined /> </a> </b>
                    <b>Công nợ </b>
                </td>
                <td className="_bold">
                ...
                content={
                    <div>
                        ...
                        <Modal title="Xác nhận xóa" 
                            open={isModalXoaOpen} 
                            onOk={async () => {
                                    const result = await callApi(route('hoa_don.huyHoaDon.traHangNhap', [idAction]));
