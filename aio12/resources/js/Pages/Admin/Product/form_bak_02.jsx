...
export default function Dashboard(props) {
    ...
    // const [donViQuyDoi, setDonViQuyDoi] = useState(props.product && props.product.don_vi_quy_doi_id ? props.product.don_vi_quy_doi_id : null);
    const [donViQuyDoi, setDonViQuyDoi] = useState();
    const [checkDonVuQuyDoi, setCheckDonVuQuyDoi] = useState('');
    ...
    function formTonKho() {
        ...
        for (let key in tonKhoDetail) {
            result.push(
                 <tr>
                    <td>{tonKhoDetail[key].chi_nhanh_name}</td>
                    <td>
                        <InputNumber min={0} value={tonKhoDetail[key].ton_kho}  onChange={(val) => { 
                                ...
                                <Col sm={{ span: 24 }} className={'main-item-form-02 ' + classItem.dinh_muc_ton_it_nhat}>
                                    <div className="sub-main-item-form-02">
                                        ...
                                        <Row>
                                            ...
                                            {/* đơn vị quy đổi */}
                                            <Col sm={24} md={8} className={'sub-main-item-form-image ' + classItem.don_vi_tinh }>
                                                <b>Đơn vị quy đổi {donViChinh}</b>
                                                <Form.Item className="item-form" name='don_vi_quy_doi_id' label=''>
                                                    <Select
                                                        disabled={!donViChinh}
                                                        style={{ width: '100%' }}
                                                        onChange={(val) => {
                                                            if(val === donViChinh) {
                                                                setCheckDonVuQuyDoi('Đơn vị quy đổi phải khác đơn vị chính');
                                                                formProduct.setFieldValue('don_vi_quy_doi_id', null);
                                                                return false;
                                                            }
                                                            setCheckDonVuQuyDoi('');
                                                            setDonViQuyDoi(val);
                                                ...
                                                <em className="_red">{checkDonVuQuyDoi}</em>
                                            </Col>

                                            {/* Giá trị quy đổi */}
                                            <Col sm={24} md={8} className={'sub-main-item-form-image ' + classItem.don_vi_tinh }>
                                                <b>Giá trị quy đổi</b>
                                                <Form.Item className="item-form" name='gia_tri_quy_doi' label=''>
                                                    <InputNumber disabled={!donViQuyDoi} prefix={ props.donVi_key[donViChinh] ? '01 ' + props.donVi_key[donViChinh] + ' = ' : ''} min={0} suffix={props.donVi_key[donViQuyDoi]} />
                                                </Form.Item>
                                            </Col>
                                        </Row>
                                    </div>
