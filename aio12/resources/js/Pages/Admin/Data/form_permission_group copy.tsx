...
import { itemMenu, tblConfig } from "../../../Function/config_route";
import { perConfig } from "../../../Function/config_permission";
import ImgCrop from 'antd-img-crop';

import { checkRule, showData, showDataSelectTable } from '../../../Function/data';

import { parseJson, numberFormat02, showInfo } from '../../../Function/common';
...
export default function Dashboard(props) {
    ...
    function permissionData() {
        let idx = 0;
        const content = Object.entries(perConfig).map(([key, value]) => {
            ...
            return <tr key={idx}>
                <td>{value.name}</td>
                <td>
                    <table className='sub-table-permission'>
                        <thead className={`ant-table-thead thead-permission ${idx > 1 ? '_hidden' : ''}`}>
                            <tr>
                                ...
                                <th className="ant-table-cell" scope="col">User {showInfo('Chỉ được xem những nội dung do user đó tạo ra')}</th>
                        ...
                        <tbody>
                            {value.sub.map((val) => {
                                return <tr>
                                    <td>{val.name}</td>
                                    <td>
                                        <Checkbox onChange={(e) => { onChangePermission(e, 'table_view') }} defaultChecked={permissionStatus('table_view', 'web_config')}></Checkbox>
                                    </td>
                                    <td>
                                        <Checkbox onChange={(e) => { onChangePermission(e, 'table_add') }} defaultChecked={permissionStatus('table_add', 'web_config')}></Checkbox>
                                    </td>
                                    <td>
                                        <Checkbox onChange={(e) => { onChangePermission(e, 'table_edit') }} defaultChecked={permissionStatus('table_edit', 'web_config')}></Checkbox>
                                    </td>
                                    <td>
                                        <Checkbox onChange={(e) => { onChangePermission(e, 'table_delete') }} defaultChecked={permissionStatus('table_delete', 'web_config')}></Checkbox>
                                    </td>
                                    <td>
                                        <Checkbox onChange={(e) => { onChangePermission(e, 'table_create_by') }} defaultChecked={permissionStatus('table_create_by', 'web_config')}></Checkbox>
                                    </td>
                                </tr>
                            })}
                        </tbody>
                ...
                <div className="ant-table-content">
                    ...
                    <table>
                        <thead className="ant-table-thead thead-permission">
                            <tr>
                                <th className="ant-table-cell" scope="col">Nhóm Chức năng</th>
                                <th className="ant-table-cell" scope="col">Phân quyền</th>
                            </tr>
                        </thead>
                        {permissionData()}
                    </table>
